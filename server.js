...
app.get('/', (req, res) => res.redirect('/moderator.html'));
  ...
  try{
    ...
    if (Array.isArray(arr) && arr.length){
      return arr.map(pl => ({
        ...
        tracks: Array.isArray(pl.tracks) ? pl.tracks.filter(t => typeof t === 'string' && t.trim()) : []
...
function nextTrack(room){
  ...
  if (room.trIndex + 1 < curPL.tracks.length){
    room.trIndex += 1; return true;
  } else if (room.plIndex + 1 < room.playlists.length){
    room.plIndex += 1; room.trIndex = 0; return true;
  }
  return false;
}
