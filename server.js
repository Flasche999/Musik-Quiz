...
app.get('/', (req, res) => res.sendFile(path.join(__dirname, 'public', 'moderator.html')));
...
// Tracks können String oder {src,title} sein → normalize
...
function loadPlaylists(){
  ...
  // Fallback
  return [{ name:'Playlist 1', tracks:[ {src:'sounds/song1.mp3', title:'Song 1'}, {src:'sounds/song2.mp3', title:'Song 2'} ]}];
}
...
// ─── Spiel-Logik ───────────────────────────────────────────
const rooms = {};
const genCode = () => Array.from({length:5},()=> 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'[Math.floor(Math.random()*33)]).join('');
...
function setRound(room, plIdx){
  ...
  room.trIndex = Math.max(0, len - 1); // Start bei Song N
}
...
const pointsForCurrent = (room) => room.trIndex + 1;
...
io.on('connection', (socket) => {
  ...
  socket.on('mod:result', ({type}) => {
    ...
    const points = pointsForCurrent(room); // Punkte = Songnummer
